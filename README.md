...
Color c = Color::Red;  // обязательно с Color::
int x = Color::Green;  // ошибка! нет неявного приведения к int
```
- перечислители внутри области видимости (Color::Red) → меньше конфликтов
- нет неявного преобразования к int → сильнее типобезопасность

Перечисление хранится как целое некоторого типа (например, int), но в C++ можно его зафиксировать:
```
...
```
Когда это важно:
- бинарные форматы/сеть/файлы
- экономия памяти (например, миллионы элементов)
- ABI/совместимость

У enum class нет неявного преобразования к int, поэтому делаем явно:
```
auto v = static_cast<int>(Color::Green);
```

enum отлично ложится на switch:
```
...
```
Про default
- Если вы не ставите default, компилятор иногда может предупреждать о неохваченных перечислителях — это полезно при расширении enum.
- Если ставите default, компилятор может перестать предупреждать о “забытых” новых значениях.

 В C++ можно объявить перечисление заранее — удобно для заголовков:
```
enum class Token : int; // предобъявление с фиксированным типом
